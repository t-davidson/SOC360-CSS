...
round(as.numeric(X[,unlist(lookup.index.from.token["monster"])]),5)[1:100]
```
...
round(as.numeric(words.lsa[,unlist(lookup.index.from.token["fight"])]), 2)
```
...
bind_cols(names(get.top.n("fight",sims)), names(get.top.n("fight",sims.lsa)))
```
...
## Comparing similarities
```{r, echo=TRUE, tidy=FALSE, mysize=TRUE, size='\\footnotesize'}
bind_cols(names(get.top.n("ghost",sims, n = 5)), names(get.top.n("ghost",sims.lsa, n = 5)))
```
