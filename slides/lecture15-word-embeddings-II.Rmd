...
model <- word2vec::word2vec(x = tolower(df$text), 
                  ...
                  iter = 10L, # train over 100 iterations
                  negative= 10L,# 10 negative samples for each positive sample
                  min_count = 10L) # Drop words less than 3 times
```
...
```{r, echo=TRUE, tidy=FALSE, mysize=TRUE, size='\\footnotesize'}
vector <- emb["chicago", ] - emb["illinois", ] + emb["california", ]
predict(model, vector, type = "nearest", top_n = 10)
```
...
# Word embeddings
## Similarities
Find the top 5 most similar terms to "love" in the embedding space.
```{r, echo=TRUE, tidy=FALSE, mysize=TRUE, size='\\footnotesize', eval = FALSE}
...
# Word embeddings
## Similarities
Find the top 5 most similar terms to "hamlet" in the embedding space.
```{r, echo=TRUE, tidy=FALSE, mysize=TRUE, size='\\footnotesize', eval = FALSE}
