...
# Course updates
- Homework 2 grades and proposal feedback will be released on Wednesday
- Homework 3 also released Wednesday, due 4/1
...
# Latent Dirichlet Allocation
## Preprocessing
Removing hashtags, mentions and URLs using a regular expression. I initially left in hashtags and mentions but found that they tended to dominate some topics since different newspapers frequently used the same hashtags. I also used some keywords related to the Russian invasion of Ukraine, dropping around half the tweets.
```{r, eval=FALSE, echo=TRUE, tidy=FALSE, mysize=TRUE, size='\\footnotesize'}
data <- data %>% 
...
data <- data %>% 
    filter(!grepl("Ukrain|Russia|Kiev|Kyiv|Moscow|Putin|Zelensky", text))
```
...
# Latent Dirichlet Allocation
## Analyzing topics
We can use the `tidy` function to pull the beta matrix from the model. Each row corresponds to the probability of word $w$ in topic $k$. In this case we can see the probabilities of the term "biden" in topics 1-5.
```{r,eval=FALSE, echo=TRUE, mysize=TRUE, size='\\footnotesize'}
topics <- tidy(news_model, matrix = "beta")
topics %>% filter(term == "biden") %>%
...
```{r, eval=FALSE,echo=FALSE, tidy=FALSE, mysize=TRUE, size='\\footnotesize'}
topic <- 1 # Specify a topic

get_top_docs_and_words <- function(t, n = 5) {
    top_docs <- documents %>% arrange(desc(gamma)) %>% dplyr::filter(topic == topic) %>%
      left_join(data, by = c("document" = "status_id")) %>% slice_max(gamma, n= n) %>%
    dplyr::select(text)
    
    top_words <- topics %>% filter(topic == topic) %>%
            top_n(n, beta) %>% dplyr::select(term)
    
    return(c(top_docs, top_words))
} 

get_top_docs_and_words(topic)
```
