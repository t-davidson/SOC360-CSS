...
# Course updates
- Homework 2 grades and proposal feedback released later today
- Homework 3 will be released on Thursday
    - Introduction to NLP
    - Word embeddings
    - Topic modeling
...
data <- as_tibble(read_csv("../data/sotu_texts.csv")) %>%
    sample_frac(0.5)
data <- data %>% distinct(paragraph, .keep_all = TRUE) # Removing any duplicates
data$index <- 1:nrow(data) # Create a unique index
```
...
word.counts <- data %>% unnest_tokens(word, paragraph, strip_punct = TRUE) %>%
...
## Inspecting topics
```{r, eval=FALSE,echo=FALSE, tidy=FALSE, mysize=TRUE, size='\\footnotesize'}
documents$document <- as.numeric(documents$document) # Setting to ensure indexes match
get_top_docs_and_words(2, n_docs = 5)
```
