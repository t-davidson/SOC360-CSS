...
```{r, echo=TRUE, tidy=FALSE, mysize=TRUE, size='\\footnotesize'}
precision <- bind_cols(data_training, preds) %>% precision(truth=degree, estimate = .pred_class)
recall <- bind_cols(data_training, preds) %>% recall(truth=degree, estimate = .pred_class)
...
```{r, echo=TRUE, tidy=FALSE, mysize=TRUE, size='\\footnotesize'}
precision <- bind_cols(data_testing, preds) %>% precision(truth=degree, estimate = .pred_class)
recall <- bind_cols(data_testing, preds) %>% recall(truth=degree, estimate = .pred_class)
print(bind_rows(precision, recall))
```
