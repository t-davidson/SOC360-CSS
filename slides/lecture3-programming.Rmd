...
- If-else statements allow us to chain together one or more conditional actions.
  - e.g. If time is between 12:10-1:30 pm AND day is Monday or Thursday,  attend Computational Social Science lecture. Else, do something else.
...
# If-else statements
In this case we have a vector containing five fruit. We use `sample` to randomly pick one. We can use an if-statement to determine whether we have selected an apple.
```{r, echo=TRUE, tidy=TRUE, mysize=TRUE, size='\\footnotesize'}
fruit <- c("apple", "apple",
            "orange", "orange",
            "apple")
...
# Exercise: If-else
Write an if-else statement to print `TRUE` if the sum of `y` is greater than 0. Otherwise, print false.
```{r, echo=TRUE, tidy=TRUE, mysize=TRUE, size='\\footnotesize'}
y <- rnorm(10)
...
# For-loops
```{r, echo=TRUE, tidy=TRUE, mysize=TRUE, size='\\footnotesize'}
is_orange <- logical(length(fruit)) # a vector of logical objects
...
# Exercise: For-loops
Write a loop to print the values in each row of `M`. Each iteration should print out the vector representing an entire row.
```{r, echo=TRUE, tidy=TRUE, mysize=TRUE, size='\\footnotesize'}
...
# While-loops
In this example we iterate over fruit until we get a pineapple.
```{r, echo=TRUE, tidy=TRUE, mysize=TRUE, size='\\footnotesize'}
i <- 1 # iterator
f <- fruit[i] # define initial value 
while (f != "pineapple") {
  print(f)
  i <- i+1 # increment index
  f <- fruit[i] # get next f
}
```
