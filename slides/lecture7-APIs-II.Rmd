...
# Using the NYT API
## Inspecting the results
The data are returned in a nested list. Indexing can be used to extract different components of the results.
```{r, echo=TRUE, tidy=FALSE, mysize=TRUE, size='\\footnotesize', eval = F}
...
# Using the NYT API
## Inspecting the results
However, due to the structure of the data we cannot easily cast to a tibble.
```{r, echo=TRUE, tidy=FALSE, mysize=TRUE, size='\\footnotesize', eval = F}
articles %>% as_tibble()
```
...
# Using the NYT API
## Cleaning the data
I made a function to extract the relevant information into a tibble. First, it applies the function to each row, then uses `bind_rows` to render the tibble.
```{r, echo=TRUE, tidy=FALSE, mysize=TRUE, size='\\footnotesize', eval = F}
