...
drake <- get_artist_audio_features("Drake") %>% as_tibble()
head(drake)
```
...
p <- ggplot(drake, aes(x=album_release_year, y=danceability))
p + geom_smooth() + 
  labs(title="Danceability of Drake over time", caption = "Data from collect from Spotify API") +
...
## Visualizing the data
```{r, echo=FALSE, tidy=FALSE, mysize=TRUE, size='\\footnotesize'}
p <- ggplot(drake, aes(x=album_release_year, y=acousticness))
p + geom_smooth() +
  labs(title="Acousticness of Drake over time", caption = "Data from collect from Spotify API")  +
...
both <- bind_rows(drake, taylor) # adding TS to the same tibble
...
p + geom_point(alpha=0.1) + geom_smooth() +
  labs(title="Comparing danceability of Drake and Taylor Swift", caption = "Data from collect from Spotify API") +
  xlab("") + ylab("Mean danceability") + theme_bw()
```  
...
p + geom_point(alpha=0.1) + geom_smooth() +
  labs(title="Comparing acousticness of Drake and Taylor Swift", caption = "Data from collect from Spotify API") +
  xlab("") + ylab("Mean acousticness") + theme_bw()
```  
